#
```
https://github.com/PacktPublishing/Deep-Learning-with-TensorFlow-2-and-Keras
```
```
https://github.com/brightmart/text_classification


https://www.itread01.com/content/1546397464.html

https://www.itread01.com/content/1557387604.html

https://github.com/Apm5/tensorflow_2.0_tutorial/tree/master/RNN
```
```
以 colab進行gensim 訓練中文詞向量
https://home.gamer.com.tw/creationDetail.php?sn=4625319

在Colab中利用gensim的Word2Vec建立RNN要使用的Word Embedding
https://www.youtube.com/watch?v=U2TT5qgCTdk

https://github.com/zake7749/word2vec-tutorial

```
#
```
文本分类实战（十）—— BERT 预训练模型
https://github.com/jiangxinyang227/textClassifier
1 大纲概述

　　文本分类这个系列将会有十篇左右，包括基于word2vec预训练的文本分类，与及基于最新的预训练模型（ELMo，BERT等）的文本分类。总共有以下系列：

　　word2vec预训练词向量

　　textCNN 模型

　　charCNN 模型

　　Bi-LSTM 模型

　　Bi-LSTM + Attention 模型

　　RCNN 模型

　　Adversarial LSTM 模型

　　Transformer 模型

　　ELMo 预训练模型

　　BERT 预训练模型

　　所有代码均在textClassifier仓库中。

 

2 数据集

　　数据集为IMDB 电影影评，总共有三个数据文件，在/data/rawData目录下，包括unlabeledTrainData.tsv，labeledTrainData.tsv，testData.tsv。
  在进行文本分类时需要有标签的数据（labeledTrainData），
  数据预处理如文本分类实战（一）—— word2vec预训练词向量中一样，预处理后的文件为/data/preprocess/labeledTrain.csv
```
#
```
Tensorflow 2.0 Tutorial - What is an Embedding Layer? Text Classification P2
https://www.youtube.com/watch?v=qpb_39IjZA0

https://github.com/guillaume-chevalier/GloVe-as-a-TensorFlow-Embedding-Layer
```
#
```
https://github.com/aymericdamien/TensorFlow-Examples/tree/master/tensorflow_v2/notebooks/2_BasicModels

word2vec.ipynb


https://www.cnblogs.com/jiangxinyang/p/9332769.html
```
# GloVe Vectors Embedding
```
https://www.youtube.com/watch?v=YNK-pBXDLzk

https://github.com/guillaume-chevalier/GloVe-as-a-TensorFlow-Embedding-Layer

https://www.itread01.com/content/1546749363.html
```

# fasttext tensorflow 2
```

https://towardsdatascience.com/full-nlp-use-case-with-fasttext-and-tensorflow-2-748381879e33


https://nbviewer.ipython.org/github/tonio73/data-science/blob/master/nlp/DownloadC2cRoutes.ipynb

https://nbviewer.ipython.org/github/tonio73/data-science/blob/master/nlp/C2C_Fasttext-Tensorflow.ipynb

https://blog.csdn.net/linchuhai/article/details/86648074

https://blog.csdn.net/u010417185/article/details/80651716

https://blog.csdn.net/u010417185/article/details/80651716


https://blog.csdn.net/feilong_csdn/article/details/88655927
https://fasttext.cc/docs/en/unsupervised-tutorial.html
```
#
```
https://www.udemy.com/course/nlp-with-bert-in-python/?referralCode=063516494616C76907CD
```
